...
impl Value {
    pub fn unwrap_int(self, expr: &Located<Expr>) -> Result<i32, ComptimeError> {
        if let Value::Int(n) = self {
            Ok(n)
        } else {
            Err(ComptimeError::type_mismatch("Int", self.type_name(), expr))
        }
    }
}

impl ShowError for ComptimeError {
