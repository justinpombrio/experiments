use parser_ll1::{choice_2, choice_6, recur, seq_3, Grammar, GrammarError, ParseError, Parser};
...
fn make_json_parser() -> Result<impl Fn(&str) -> Result<Json, ParseError>, GrammarError> {
    ...
    let json_p = recur("json", &mut |json_p| {
        // Null
        ...
        let number_p = g
            ...
            .try_span(|span| f64::from_str(span.substr).map_err(|e| e.to_string()))
            .map(Json::Number);
        ...
        // Objects
        let entry_p =
            seq_3(plain_string_p, g.string(":")?, json_p.clone()).map(|(key, _, val)| (key, val));
